...
### Язык клиентских отчётов / КП / брифов
**ОБЯЗАТЕЛЬНО:** перед сдачей любого клиентского отчёта, КП или брифа применять чек-лист `knowledge/team-playbooks/report-style.md` (методология Смарина). Ключевое: пишем для заказчика, без жаргона, API-кодов, хэшей, ссылок на параграфы методологии и наших внутренних терминов («маркетолог», «псевдо-CPA», «фрод», «структурный мусор» и т.д.). Открытые вопросы выносим явно, не угадываем решения за клиента.
